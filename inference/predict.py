...
class PredictSentiment():
    def __init__(self) -> None:
        with open(os.path.join("models", "best_model_mlp.pkl"), 'rb') as f:
            self.model_mlp = pickle.load(f)
        self.model_lstm = load_model(os.path.join("models","model_lstm.h5"))       
        pass

    def predict_text_ann(self,bow) -> np.int64:
        return self.model_mlp.predict(bow)[0]

    def predict_text_lstm(self,input_ids) -> np.int64:
        input_ids = np.reshape(input_ids, (1,128))
